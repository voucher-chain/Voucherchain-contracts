...
contract VoucherChain is Ownable, ReentrancyGuard {
    ...
    event VoucherReclaimed(bytes32 indexed voucherHash, address indexed token, address indexed agent, uint256 tokenValue);
    ...
    function mintVoucherBatch(
        VoucherBatch memory batch
    ) external onlyAuthorizedMinter nonReentrant {
        if (batch.voucherHashes.length == 0) revert InvalidBatchSize();
        if (batch.voucherHashes.length != batch.tokens.length || 
            batch.voucherHashes.length != batch.tokenValues.length || 
        ...
        for (uint256 i = 0; i < batch.voucherHashes.length; i++) {
            ...
            emit VoucherMinted(batch.voucherHashes[i], batch.tokens[i], batch.tokenValues[i], msg.sender, expiryTimestamp);
        }
    ...
    function reclaimExpiredVoucher(
        string memory voucherCode
    ) external nonReentrant {
        bytes32 voucherHash = keccak256(abi.encodePacked(voucherCode));
        Voucher storage voucher = vouchers[voucherHash];
